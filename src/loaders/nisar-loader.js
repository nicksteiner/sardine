...
export async function loadNISARRGBComposite(fileOrUrl, options = {}) {
  ...
  async function getRGBTile({ x, y, z, bbox, multiLook = false, noCache = false }) {
    ...
    try {
      ...
      // ── Full-resolution path — always read actual chunks ──
      // Worker pool parallelizes decompression across cores, so no need for
      // a coarse mosaic shortcut. This eliminates aliasing artifacts.
      const stepX = sliceW / tileSize;
      const stepY = sliceH / tileSize;
...
export async function loadNISARGCOVFromUrl(url, options = {}) {
  ...
  const getTile = async ({ x, y, z, bbox, multiLook, signal, background = false }) => {
    ...
    try {
      ...
      if (_refinementEnabled && tileInScope && (coarseStrideR > fineStrideR || coarseStrideC > fineStrideC)) {
