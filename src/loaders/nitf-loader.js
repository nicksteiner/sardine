 ...
 * needed for the current viewport tile. Decoder dispatch is driven by
 * SICD ImageData/PixelType (standard, vendor-neutral) when SICD XML
 * is present, with a fallback to NITF-header inference for non-SICD
 * NITFs. Per the SICD Volume 1 standard three PixelType values are
 * supported:
 *
 *   RE32F_IM32F  — complex float32 I/Q, 8 bytes/pixel
 *   RE16I_IM16I  — complex int16 I/Q,   4 bytes/pixel
 *   AMP8I_PHS8I  — magnitude+phase uint8, 2 bytes/pixel
 *
 * All paths output radiometric power (I² + Q² or amp²) so the shader's
 * 10·log10() yields correct dB. Phase is intentionally discarded — this
 * is an intensity viewer.
 *
 * Designed for uncompressed (IC=NC/NM) NITF files. JPEG 2000 compressed
...
/**
 * Decode complex float32 pixel-interleaved data (SICD RE32F_IM32F).
 * Output = SF × (I² + Q²) — radiometric power.
 * Big-endian per NITF 2.1.
 */
function decodeComplexFloat32P(dv, out, outW, outH, ncols, startCol, stride, rowStride, maxSrcRows, sf) {
  const rowBytes = ncols * 8; // 2 bands × 4 bytes each
  for (let r = 0; r < outH; r++) {
    const srcRow = r * rowStride;
    if (srcRow >= maxSrcRows) break;
    const rowByteOff = srcRow * rowBytes;
    for (let c = 0; c < outW; c++) {
      const srcCol = startCol + c * stride;
      if (srcCol >= ncols) break;
      const pixByteOff = rowByteOff + srcCol * 8;
      const i = dv.getFloat32(pixByteOff, false);
      const q = dv.getFloat32(pixByteOff + 4, false);
      out[r * outW + c] = sf * (i * i + q * q);
    }
  }
}

/**
 * Decode amplitude/phase uint8 pixel-interleaved data (SICD AMP8I_PHS8I).
 * Output = SF × amp² — power. Phase byte is ignored (intensity viewer).
 */
function decodeAmp8Phs8P(dv, out, outW, outH, ncols, startCol, stride, rowStride, maxSrcRows, sf) {
  const rowBytes = ncols * 2; // 2 bands × 1 byte each
  for (let r = 0; r < outH; r++) {
    const srcRow = r * rowStride;
    if (srcRow >= maxSrcRows) break;
    const rowByteOff = srcRow * rowBytes;
    for (let c = 0; c < outW; c++) {
      const srcCol = startCol + c * stride;
      if (srcCol >= ncols) break;
      const amp = dv.getUint8(rowByteOff + srcCol * 2);
      out[r * outW + c] = sf * (amp * amp);
    }
  }
}
...
/**
 * Resolve the row-stride-in-bytes and decoder function for a given
 * NITF image + optional SICD metadata. Dispatch is driven by the
 * standard SICD PixelType field when present (vendor-neutral), with
 * fallback to NITF header fields (pvtype/nbpp/bands) for non-SICD NITFs.
 *
 * Returns { bytesPerPixel, decode } where decode has the same signature
 * as the decoders above (last `sf` arg is ignored by non-complex paths).
 * Throws if the pixel layout can't be resolved.
 */
function resolvePixelLayout(img, sicd) {
  const pt = sicd?.pixelType;

  if (pt === 'RE32F_IM32F') return { bytesPerPixel: 8, decode: decodeComplexFloat32P };
  if (pt === 'RE16I_IM16I') return { bytesPerPixel: 4, decode: decodeComplexInt16P };
  if (pt === 'AMP8I_PHS8I') return { bytesPerPixel: 2, decode: decodeAmp8Phs8P };

  // Non-SICD or SICD without PixelType: fall back to NITF header inference.
  const isComplexIQ = img.totalBands === 2
    && img.bands[0]?.isubcat === 'I' && img.bands[1]?.isubcat === 'Q'
    && img.imode === 'P';
  if (isComplexIQ && img.pvtype === 'R' && img.nbpp === 32) {
    return { bytesPerPixel: 8, decode: decodeComplexFloat32P };
  }
  if (isComplexIQ && img.pvtype === 'SI' && img.nbpp === 16) {
    return { bytesPerPixel: 4, decode: decodeComplexInt16P };
  }
  if (img.pvtype === 'R' && img.nbpp === 32 && img.totalBands === 1) {
    return { bytesPerPixel: 4, decode: decodeFloat32 };
  }
  if ((img.pvtype === 'SI' || img.pvtype === 'INT') && img.nbpp === 16 && img.totalBands === 1) {
    return { bytesPerPixel: 2, decode: decodeInt16 };
  }

  throw new Error(
    `Unsupported pixel layout: pvtype=${img.pvtype} nbpp=${img.nbpp} ` +
    `bands=${img.totalBands} imode=${img.imode} ` +
    `sicd.pixelType=${pt ?? '(none)'}`
  );
}
...
export async function loadNITF(file, onProgress) {
  ...
  async function getTile({ x, y, z, bbox }) {
    try {
      ...
      if (stride <= 8) {
        ...
        decode(dv, out, outW, outH, ncols, pxLeft, stride, stride, readRowCount, calSF);
      } else {
        ...
        for (let r = 0; r < outH; r++) {
          ...
          decode(dv, out.subarray(r * outW), outW, 1, ncols, pxLeft, stride, 1, 1, calSF);
        }
      }
  ...
  async function readRegion(startRow, numSrcRows, startCol, numSrcCols) {
    ...
    decode(dv, out, outW, outH, ncols, startCol, 1, 1, clampedRows, calSF);
